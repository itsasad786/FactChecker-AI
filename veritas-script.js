...
async function analyzeText() {
    ...
    try {
        ...
        // Call backend API (Node server) instead of browser-side Gemini
        ...
        showNotification('Showing demo results. Please ensure the backend server is running.', 'warning');
...
async function analyzeURL() {
    ...
    try {
        // Step 1: Extract content from URL (client-side)
...
${contentToAnalyze || 'No content extracted'}
        ...
        // Step 3: Analyze with backend API (Node server)
        const analysisTypes = [
        ...
        const response = await fetch('/api/analyze', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ text: contentText, analysisTypes })
        });

        const analysisResult = await response.json();

        if (!response.ok || !analysisResult || !analysisResult.success) {
...
async function analyzeFile(fileIndex) {
    ...
    try {
        ...
        // Optional: show mock results if backend is unavailable
        showNotification('Showing demo results. Please ensure the backend server is running.', 'warning');
...
async function testAPIConnection() {
    try {
        ...
        if (response.ok && result.success) {
            ...
            showNotification(`API connection failed: ${result.message || 'Unknown error'}`, 'error');
...
async function analyzeExtractedFileContent(extractionResult) {
    try {
        ...
        const response = await fetch('/api/analyze', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ text: contentToAnalyze, analysisTypes: textAnalysisTypes })
        });
