...
- **Google Gemini AI** - Advanced language model for content analysis
  - Gemini 3.0 for fast, accurate analysis
  - If for any reason all Gemini 3.0 models fail, it automatically fallback to secondary Gemini Models
...
2. **Configure API Key**
   ...
   - Creat environmenet variable in Node (.env file)
   - Set the key 'Gemini_API_KEY' and its corresponding value (separated by = sign)

3. **Run Locally**
   - Open Terminal and install node js (if not installed)
   - Install required dependencies:
     ```bash
     npm install # if not already done
     npm run dev
     ```
   - Go to http:localhost:YOUR_PORT_NUMBER in your browser
