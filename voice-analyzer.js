...
async function analyzeVoice() {
    ...
    try {
        ...
        showNotification('Starting voice analysis...', 'info');
...
Voice Note Analysis Request:

Transcription: ${transcription}

Please analyze this voice content for:
1. Content credibility and factual accuracy
2. Emotional tone and manipulation techniques
3. Language quality and bias detection
4. Overall trustworthiness assessment

Provide a comprehensive analysis with specific scores and recommendations.
        `.trim();

        // Analyze using Gemini API
        const result = await geminiAPI.analyzeText(contentToAnalyze);
        
        if (result.success) {
            showNotification('Voice analysis completed successfully!', 'success');
            showVoiceAnalysisResults(result.data, transcription);
        } else {
        ...
        showVoiceAnalysisResults(generateMockVoiceResults(), transcription);
...
// Test voice API - Make it globally available
window.testVoiceAPI = async function() {
    console.log('testVoiceAPI called'); // Debug log
    ...
    try {
        ...
        if (typeof geminiAPI === 'undefined') {
            console.log('geminiAPI not available');
            showNotification('API module not loaded. Please refresh the page.', 'error');
            return false;
        }
        
        console.log('Showing info notification...');
        showNotification('Testing voice analysis API...', 'info');
        
        console.log('Calling geminiAPI.testConnection()...');
        const result = await geminiAPI.testConnection();
        console.log('API test result:', result);
        
        if (result.success) {
            console.log('API test successful, showing success notification');
            showNotification('Voice analysis API connection successful!', 'success');
            return true;
        } else {
            console.log('API test failed, showing error notification');
            showNotification(`Voice analysis API connection failed: ${result.message}`, 'error');
            return false;
        }
    } catch (error) {
        console.error('Voice API test error:', error);
        showNotification(`Voice analysis API test failed: ${error.message}`, 'error');
        return false;
    }
};
